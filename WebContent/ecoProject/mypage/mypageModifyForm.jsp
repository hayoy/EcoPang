		...
		<div class="signupTitle">회원 정보 수정</div>
			<form action="mypageModifyPro.jsp" method="post" name="inputForm"
				...
				<table class="table3">
					...
					<!-- 밑에부터는 선택항목 -->
					<tr>
						<td colspan="2">프로필 사진 (선택)</td>
					</tr>
					<tr>
						<td rowspan="2">
							<%if (user.getUser_img() != null) {// 사진저장된게 있을경우%> 
								<img src="/ecopang/ecoProject/imgs/<%=user.getUser_img()%>" id="previewImg"  width="150" />
							<%} else {%> 
					...
					<tr>
						<!-- 비밀번호 확인후 정보 변경 -->
						<td colspan="2" style="padding-top:10px;"><br/><br/><br/>비밀번호 확인후 정보수정이 가능합니다!<br/> 비밀번호를 입력해 주세요 !!! (필수 입력)<br/><input type="password" name="pwch" class="text" style="margin-left:80px;"/></td>
					</tr>
					<tr>
